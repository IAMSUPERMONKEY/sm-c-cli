...
5. 合并所有成功响应中的 `data.list`，按 `scheduleId` 去重，再按 `scheduleDate`、`startTime` 升序排列。
6. 如果用户指定的是精确日期范围，而不是完整自然月，使用 `scheduleDate` 再次过滤合并结果，只保留起止日期内的记录。边界月份也必须执行这一步。
...
| `className`             | 课程名称                                   |
| `trainerName`           | 教练昵称                                   |
| `trainingType`          | 运动类型：`1` 团体课、`2` 训练营、`3` 私教 |
| `nonStart`              | 未开课状态：`0` 已开课、`1` 未开课         |
| `checkin`               | 签到状态：`0` 未签到、`1` 已签到           |
| `isWait`                | 是否等候中：`0` 否、`1` 是                 |
| `scheduleId`            | 课表 id，可用于合并结果去重                |

向用户展示结果时，优先使用日期、时间、门店、课程、教练、运动类型和状态等人类可读信息；除非用户明确要求，不展示 `*IdSk`、头像地址等内部字段。

## 结果汇总

- 用户只要求“记录列表”时，按时间展示合并后的记录。
- 用户询问运动次数时，以过滤、去重后的记录条数为准。
- 用户询问常去门店、常上课程、教练或运动类型时，基于过滤、去重后的记录分组统计，不要根据单个月份外推。
