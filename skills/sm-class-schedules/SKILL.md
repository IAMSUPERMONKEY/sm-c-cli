...
- `--city`（必填）：城市名称，**必须**是超级猩猩业务覆盖城市白名单内的城市，且使用「\*\*市」格式。
  - 完整白名单与使用规则见 [`../sm-shared/references/cities.md`](../sm-shared/references/cities.md)，调用前请先读取该文件确认城市是否在白名单内。
...
- **城市必须明确，且必须在业务覆盖白名单内**。城市白名单见 [`../sm-shared/references/cities.md`](../sm-shared/references/cities.md)；如果用户没说城市，**先让用户发送当前位置或询问用户城市**，不要猜。
